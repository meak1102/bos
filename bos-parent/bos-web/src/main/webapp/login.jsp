...
<script type="text/javascript">
	...
	function checkCode(){
		...
		if(checkCode==null||checkCode==""||checkCode.trim()==""){
			$("#span3").html("验证码不能为空").css("color","red");
		...
		$.post("${pageContext.request.contextPath }/userAction_checkCode",{"checkCode":checkCode},function(data){
			if(data){
				...
				$("#span3").html("验证码错误").css("color","red");
				checkCode_flag=false;
			}			
		});	
	}
	$(function(){
		$("a[name='loginform:j_id19']").click(function(){
			var flag=$("#loginform").form("validate");
			if(flag&&checkCode_flag){
				$("#loginform").submit();
			}
		});
		$("input[name='checkcode']").blur(checkCode);
					...
					<div id="idInputLine" class="loginFormIpt showPlaceholder"
						...
						<input id="loginform:idInput" type="text" name="email"
							class="loginFormTdIpt easyui-validatebox" maxlength="50"
							 data-options="required:true,validType:['email','length[5,20]']" />
						<label for="idInput" class="placeholder" id="idPlaceholder">帐号：</label>
					</div>
					<div class="forgetPwdLine"></div>
					<div id="pwdInputLine" class="loginFormIpt showPlaceholder">
						<input id="loginform:pwdInput" class="loginFormTdIpt easyui-validatebox" type="password"
							name="password" data-options="required:true,validType:'length[4,8]'" />
